...
(function () {
        ...
        class GeTuiDelegate extends GenericObject {
                ...
                static initializeApps(config) {
                        assert(_.isArray(config), 'Invalid Argument');
                        config.forEach(app => {
                                if(app.enabled !== false) {
                                        var appName = app.name;
                                        var GT = new GeTui(GT_URL, app.key, app.masterSecret);
                                        GTClients[appName] = GT;
                                        GTAppInfo[appName] = app.id;
                                        GTConfig[appName] = app;
                                        logger.info(`delegate GeTui [name=${app['name']}(${app['name-CN'] || app.name}), id=${app.id}]`);
                                }
                        });
                }

                static pushTransmission(m) {
                        return function * () {
                                EasyNode.DEBUG && logger.debug('push transmission -> ' + JSON.stringify(m));
                                m['_GT'] = GTClients[m.appName];
                                m['_APP_ID'] = GTAppInfo[m.appName];

                                var template = new TransmissionTemplate({
                                        appId: GTAppInfo[m.appName],
                                        appKey: GTConfig[m.appName].key,
                ...
                static isAppDelegated(appName) {
                        return GTClients[appName] !== undefined;
                }

                static setTag(appName, clientId, tag) {
                        var GT = GTClients[appName];
                        var APP_ID = GTAppInfo[appName];
                        return function * () {
                ...
                static bindAlias(appName, alias, clientId) {
                        var GT = GTClients[appName];
                        var APP_ID = GTAppInfo[appName];
                        return function * () {
                ...
                static unbindAlias(appName, alias, clientId) {
                        var GT = GTClients[appName];
                        var APP_ID = GTAppInfo[appName];
                        return function * () {

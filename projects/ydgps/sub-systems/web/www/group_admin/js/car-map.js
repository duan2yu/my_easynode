...
function mapView(){
   ...
   $.ajax({
    ...
    success:function(data){
      if(!data.result.lng || !data.result.lat) {
          $("#location").html("车辆位置:暂时无法定位，请稍后重试！");
          return;
      }
      ...
      /*if(data[0].gps==2){
        ...
        var spark=[data.result.lng,data.result.lat];
      positionArr=new AMap.LngLat(spark[0],spark[1]);
      map=new AMap.Map("container",{
          view: new AMap.View2D({//创建地图二维视口
              center:spark,//创建中心点坐标
              zoom:17, //设置地图缩放级别
              rotation:0 //设置地图旋转角度
             }),
...
$("#redraw").click(function(){
  redraw();
    ...
    function redraw(){
        var url="/api/device/snapshot/"+imei;
        $.ajax({
            type:"get",
            url:url,
            dataType:'json',

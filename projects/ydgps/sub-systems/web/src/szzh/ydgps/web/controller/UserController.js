        ...
        class UserController extends GenericObject {
                ...
                readUserInfo() {
                        ...
                        return function * () {
                                ...
                                delete ret[0].pwd;delete ret[0].salt;
                                var user=ret[0];
                ...
                /**
                 * @api {post} /api/user/bindDeviceGroup/:userId 用户绑定设备分组(覆盖原有绑定关系)
                 * @apiName bindDeviceGroup
                 * @apiGroup User
                 *
                 * @apiParam {Number} userId * 用户ID
                 * @apiParam {String} deviceGroupId * 分组列表，多个ID使用","分隔
                 *
                 * @apiSuccess {Number} code 返回码，0 -> 成功
                 * */
                bindDeviceGroup() {
                        ...
                        return function * () {
                                ...
                                var groupId=this.session.USER.groupId;
                                this.validator.check('deviceGroupId').necessary().end();
                                if (this.validator.isValid()) {
                                        ...
                                        if(ret.length==0)
                                        {
                                                return ActionResult.createErrorResult('用户不存在或不属于当前集团');
                                        }
                                        var deviceGroupId=this.p('deviceGroupId');
                                        var array=deviceGroupId.split(',');
                                        var ret=yield me.bookshelf.knex('user_deviceGroup')
                                                .where('userId',userId)
                                                .delete();
                                        ...
                                        {
                                                ret=yield me.bookshelf.knex('user_deviceGroup')
                                                        .insert(
                                                        {
                                                                userId:userId,
                                                                deviceGroupId:array[i]
                                                        }).transacting(tran);
                                        }
                                        tran.commit();
                                        return ActionResult.createSuccessResult();

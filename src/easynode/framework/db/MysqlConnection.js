...
(function () {
        const DEFAULT_RPP = parseInt(EasyNode.config('easynode.framework.mvc.model.defaultRPP', '20'));
        const LOW_PERFORMANCE_SQL = parseInt(EasyNode.config('easynode.framework.db.execSQLWarning', '3000'));
        ...
        class MysqlConnection extends IConnection {
                ...
                del(model, ids = [0]) {
                        var me = this;
                        assert(_.isArray(ids) || typeof ids == 'number', 'Invalid argument ids');
                        return function * () {
                                var d = new Date();
                                var sql = '';
                                if (_.isArray(ids)) {
                ...
                list(model, condition = {}, pagination = {
                        ...
                        return function * () {
                                ...
                                ret.rows = r[0].rowCount ||r[0].ROW_COUNT|| 0;
                ...
                _makeConditionParts(expression, alias, v, sql, args) {
                        ...
                        switch (expression) {
                                ...
                                case '!()' :
                                {
                                        assert(_.isArray(v) && v.length > 0, 'Invalid condition "in/not in", need Array value');

                                ...
                                if (c && c[0] != '#') {          //#is a comment flag
                                        ...
                                        if (c[0]) {
                                                if (EasyNode._config_cache[c[0]] !== undefined) {
                                                        console.log(`*** Duplicate config item [${c[0]}], value [${c[1]}] at [${file}] overwrote the others.`);
                                                }
...
EasyNode.addSourceDirectory = function () {
        ...
        arr.forEach(function (v) {
                if (_.isString(v)) {
                        if(v) {
                                assert(v.match(/.*\/src\/?$/), `Invalid source folder, a source folder should end with 'src'`);
                                var folder = EasyNode.real(v);
                                if (!fs.existsSync(folder)) {
...
EasyNode.sleep = function * (timeout) {
        function _sleep(cb) {
                setTimeout(function(){
                        cb();
                }, timeout);

...
$(function(){
  mapTime=decodeURI(ddche.GetQueryString("mapTime"))||(moment(new Date()).format("YYYY-MM-DD")+" 00:00:00");
  $("#lineTime").val(mapTime);
  ...
  $.ajax({
    ...
    data:{
      beginTime:mapTime,
      endTime:moment(new Date(mapTime).getTime()+60*60*24*1000).format("YYYY-MM-DD hh:mm:ss")
    },
    dataType:'json', 
 ...
 $("#chooseDay").click(function(){
    var time=$("#lineTime").val();
    if(time=="")return;
    location.href="sale-mapline.html?imei="+imei+"&mapTime="+time;
 });
});

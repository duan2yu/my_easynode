...
(function () {
        ...
        class DeviceGroupController extends GenericObject {
                ...
                /**
                 * @api {post} /api/deviceGroup/addDevice/:deviceGroupId 向设备分组中增加设备
                 * @apiName addDevice2Group
                 * @apiGroup DeviceGroup
                 *
                 * @apiParam {Number} deviceGroupId * 设备分组ID
                ...
                addDevice () {
                        ...
                        return function * () {
                                ...
                                this.validator.check('deviceId').necessary().minLength(1).end();
                                if(this.validator.isValid())
                                {
                ...
                /**
                 * @api {post} /api/deviceGroup/removeDevice/:deviceGroupId 从设备分组中删除设备
                 * @apiName removeDeviceFromGroup
                 * @apiGroup DeviceGroup
                 *
                 * @apiParam {Number} deviceGroupId * 设备分组ID
                 * @apiParam {String } deviceId * 设备ID列表，多个ID使用","分隔
                ...
                removeDevice () {
                        ...
                        return function * () {
                                ...
                                {
                                      var   ret = yield me.bookshelf.knex('device_group_relation')
                                              .andWhere('deviceGroupId',deviceGroupId)
                                              .whereRaw('deviceId in ('+this.p('deviceId')+')')
                                        .delete();

                                        if (ret && ret > 0) {
                                                return ActionResult.createSuccessResult({id: ret[0]}).setMsg('移除设备成功');
                                        }
                                        return ActionResult.createErrorResult().setMsg('移除设备失败,未找到绑定关系');
                                }
                        };
                }

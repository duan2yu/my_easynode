...
(function () {
        const CREATE_NAME = EasyNode.config('easynode.framework.mvc.model.action.createName', 'C');
        const READ_NAME = EasyNode.config('easynode.framework.mvc.model.action.readName', 'R');
        const UPDATE_NAME = EasyNode.config('easynode.framework.mvc.model.action.updateName', 'U');
        const DEL_NAME = EasyNode.config('easynode.framework.mvc.model.action.delName', 'D');
        const LIST_NAME = EasyNode.config('easynode.framework.mvc.model.action.listName', 'L');
                ...
                getCreateAction(fieldNames = '*', view = null, l = null) {
                        ...
                        class _CreateAction extends Action {
                                constructor() {
                                        ...
                                        assert(_.isArray(fieldNames), 'Invalid argument');
                                ...
                                process(ctx) {
                                        ...
                                        return function * () {
                                                ...
                                                if(l && typeof l.beforeUpdate == 'function') {
                                                        yield l.beforeUpdate.call(me, model, ctx, idVal);
                                                }
                                                var ret = yield ctx.getConnection().update(model);
                                                if(l && typeof l.afterUpdate == 'function') {
                                                        yield l.afterUpdate.call(me, model, ctx, idVal);
                                                }
                                                return ActionResult.createSuccessResult(ret);
                                        };
                                }
